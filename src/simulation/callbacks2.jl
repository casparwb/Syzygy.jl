...
function collision_callback_idiomatic!(integrator, pairs, retcode, grav_rad_multiple)
    ...
    @inbounds for pair in pairs
        ...
        collision::Bool = collision_check(
            d, Ri, Rj, Mi, Mj,
...
function check_escape!(
        integrator, retcode, candidate_index,
...
function triple_stability_callback_set(
        integrator, retcodes,
    ...
    if check_ionization
        ...
        U = -UNITLESS_G * masses * SA[
            masses[2] / d12 + masses[3] / d13,
    ...
    # Eccentricity
    return if check_democratic_ecc

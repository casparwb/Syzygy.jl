...
function keplerian_to_cartesian(elements::OrbitalElements, mass; G=GRAVCONST)
    ...
    o = QuantityArray(zeros(3), default_unit_length)
    o_dot = QuantityArray(zeros(3), default_unit_length/default_unit_time)
    x = QuantityArray(zeros(3), default_unit_length)
    v = QuantityArray(zeros(3), default_unit_length/default_unit_time)
    ...
    o[1] = r*cos(ν)
    o[2] = r*sin(ν)
    o[3] = zero(default_unit_length)

    f = √(μ*a)/r
    o_dot[1] = -f*sin(E)
    o_dot[2] = f*sqrt_1_min_e²*cos(E)
    o_dot[3] = zero(default_unit_length/default_unit_time)
...
function keplers_problem(hierarchy, masses, elements)
    ...
    n = length(masses)
    positions = QuantityArray(zeros(n, 3), default_unit_length)
    velocities = QuantityArray(zeros(n, 3), default_unit_length/default_unit_time)

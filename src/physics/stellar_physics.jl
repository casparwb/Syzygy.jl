...
function envelope_structure(star::Particle, age, Z = 0.02)
    ...
    return envelope_structure(
        star.structure.m, star.structure.R,
...
function zero_age_main_sequence_radius(M::Real)
    ...
    return (θ * M^2.5 + ι * M^6.5 + κ * M^11 + λ * M^19 + μ * M^19.5) / (ν + ξ * M^2 + o * M^8.5 + M^18.5 + Π * M^19.5)
end

function zero_age_main_sequence_radius(mass::Quantity)
    return zero_age_main_sequence_radius(ustrip(u"Msun", mass))
end
...
function main_sequence_radius_035_msun(τ::Real, Z = 0.02)
    ...
    a₁₉′ = aₙ(1.46644e+0, 1.839725e+0, 6.442199e+0, 4.023635e+0, 6.957529e-1)
    ...
    a₆₂ = aₙ(8.43e-2, -4.75e-2, -3.52e-2)
    a₇₆ = aₙ(1.192334e-2, 1.083057e-2, 1.230969e+0, 1.551656e+0)
    a₇₇ = aₙ(-1.668868e-1, 5.818123e-1, -1.105027e+1, -1.66807e+1)
    ...
    return 10^logRMS_over_RZAMS * R_zams
end
...
function convective_envelope_mass(mass, core_mass, stellar_type, age, tMS, tBGB)
    @assert stellar_type < 10 "Only stars have envelopes."
...
function main_sequence_lifetime(M::Real, Z = 0.02)
    ...
    a₁ = aₙ(1.59389e3, 2.053038e3, 1.231226e3, 2.327785e2)
    a₂ = aₙ(2.706708e3, 1.483131e3, 5.772723e2, 7.41123)
    a₃ = aₙ(1.466143e2, -1.048442e2, -6.795374e1, -1.391127e1)
    a₄ = aₙ(4.14196e-2, 4.564888e-2, 2.958542e-2, 5.571483e-3)
    a₅ = aₙ(3.426349e-1)
    a₆ = aₙ(1.949814e1, 1.758178, -6.008212, -4.470533)
    a₇ = aₙ(4.90383)
    a₈ = aₙ(5.212154e-2, 3.166411e-2, -2.750074e-3, -2.271549e-3)
    a₉ = aₙ(1.312179, -3.294936e-1, 9.23186e-2, 2.610989e-2)
    a₁₀ = aₙ(8.073972e-1)
